...
RemoteDoc.prototype.create = function (value, type, cb) {
  ...
  this.shareDoc.create(data, type, cb);
...
RemoteDoc.prototype.submitOp = function (op, cb) {
  var previous = this.get();
  this.shareDoc.submitOp(op, cb);
  return previous;
};
...
RemoteDoc.prototype._onOp = function (op) {
  ...
  if (op.length === 1) {
    ...
    // Ignore no-ops
    return;
  } else if (this.shareDoc.type !== defaultType) {
    // Ignore docs with types other then json0. They are identified by
    // being stored in collections with prefix 'pure_'
    var segments = [this.collectionName, this.id];
    var model = this.model;
    var value = this.get();
    var previous = undefined;
    var type = this.shareDoc.type;
