...
Model.prototype.create = function () {
  ...
  var segments = this._splitPath(subpath);
  return this._create(segments, value, type, cb);
};
Model.prototype._create = function (segments, value, type, cb) {
...
Model.prototype.add = function () {
  ...
  var segments = this._splitPath(subpath);
  return this._add(segments, value, type, cb);
};
Model.prototype._add = function (segments, value, type, cb) {
  if (typeof type === 'function') {
    cb = type;
    type = null;
  }
...
Model.prototype.submitOp = function () {
  var subpath, op, cb;
  if (arguments.length === 0) {
    throw new Error('Not enough arguments for submitOp');
  } else if (arguments.length === 1) {
    op = arguments[0];
  } else if (arguments.length === 2) {
    if (typeof arguments[1] === 'function') {
  ...
  var segments = this._splitPath(subpath);
  return this._submitOp(segments, op, cb);
};

Model.prototype._submitOp = function (segments, op, cb) {
  segments = this._dereference(segments);
  if (segments.length !== 2) {
    var message = 'submitOp may only be used on a document path. ' +
      'Invalid path: ' + segments.join('.');
    cb = this.wrapCallback(cb);
    return cb(new Error(message));
  }
  ...
  function submitOp(doc, docSegments, fnCb) {
    var previous = doc.submitOp(op, fnCb);
    var value = doc.get();
    var type = doc.shareDoc.type;
    var pass = model.pass({$type: {type: type, op: op}})._pass;
    model.emit('change', segments, [value, previous, pass]);
    return previous;
  }

  return this._mutate(segments, submitOp, cb);
};
